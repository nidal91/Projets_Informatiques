  ...
  if (mouseButton == LEFT){
    ...
    for (int i=1; i<img2.height-1; i++)
      for (int j=1; j<img2.width-1; j++){
        if (alpha(img2.pixels[j+i*img2.width])>128)
          img2.pixels[j+i*img2.width] = color(4*dist(j,i,mouseX,mouseY)%64,blue(img2.pixels[j+i*img2.width]),125,100);
      }
...
void mouseReleased(){
  if (iniMouseX>=0){
   ...
   for(int x=0;x<=10;x++){
     img3.beginDraw();
     PVector k = new PVector(mouseX-iniMouseX,mouseY-iniMouseY);
     float x0=iniMouseX;
     float y0=iniMouseY;
     float x1=x0;
     float y1=y0;
     color c = get(int(x0), int(y0));
     img3.stroke(c);
     for(int i=0;i<100;i++){      
         img3.line(x0,y0,x1,y1);
         x0 = x1;
         y0 = y1;
         x1=(x1+random(0, k.x/10));
         y1=(y1+random(0, k.y/10));
